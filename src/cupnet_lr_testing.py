...
for i in range(4):
    ...
    for j in range(2):
        ...
        CUPNET.compile(optimizer = Nadam(lrs[i]), loss = custom_loss, metrics = [mse_loss,ssim_loss])
        ...
        CUPNET.save_weights(f'../data/model_stuff/cupnet_weights/{j}_{lrs[i]}_cupnet_lrs_4_12.h5')
    ...
    with open("cupnet_lrs_training_logs.txt",'a') as file:
        ...
        msg = f'LR: {lrs[i]} MSE LOSS: {np.mean(inner_mse_losses)} +/- {np.std(inner_mse_losses)} SSIM LOSS: {np.mean(inner_ssim_losses)} +/- {np.std(inner_ssim_losses)} \n'
        file.write(msg)
